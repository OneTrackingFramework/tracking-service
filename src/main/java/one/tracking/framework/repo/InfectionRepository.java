...
public interface InfectionRepository extends CrudRepository<Infection, String> {
  ...
  // @Query("SELECT DISTINCT c1 FROM LocationEvent p1, LocationEvent p2, Infection i2, ContactEvent
  // c1, ContactEvent c2 "
  // + "WHERE u1.id = :userId"
  // + " AND u2.id <> :userId"
  // + " AND u2.id = i2.user.id"
  // + " AND i2.infectionType = 'INFECTED'"
  // + " AND (c1.userA = :userId OR c1.userB = :userId)"
  // + " AND (c1.locationEvent.id = p1.id)"
  // + " AND (c2.userA = u2.id OR c2.user2.id = u2.id)"
  ...
  // List<ContactEvent> findContactEvents(String userId, double distanceAlpha, int deltaTime,
